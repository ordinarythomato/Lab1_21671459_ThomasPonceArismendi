...
(define Niveles '("Básico" "Fase 1" "Fase 2" "Básico Ex" "Fase 1 Ex" "Fase 2 Ex"))
...
(define CartaPokemonVacia ;Función modeladora, que muestra la estructura y cantidad de variables de una Carta Pokémon sin considerar valores de entrada
  (lambda ()
    (list "Sin Nombre" "Normal" 0 0 "Básico" null)
    )
  )
...
(define EsCartaPokemon? ; Función verificadora para una carta Pokémon
 (lambda (CartaPokemon)
   (if (and (string? (GetNombre CartaPokemon)) ;Es el primer elemento de la Carta un string? (Nombre)
            ...
            (and (integer? (GetPS CartaPokemon))
                 (or (>= (GetPS CartaPokemon) MIN_PS)))
            ; Es el tercer elemento de la Carta, PS, un entero mayor o igual a MIN_PS? (PS: Puntos de Salud, MIN_PS: Mínimo Punto de Salud)
            (and (integer?(GetAtaque CartaPokemon))
                     (or (>= (GetAtaque CartaPokemon) MIN_ATAQUE)))
...
;; Definiendo Cartas
(define Pikachu(CartaPokemon "Pikachu" "Eléctrico" 150 60 "Básico" "Thunders"))
(define Charmander (CartaPokemon "Charmander" "Fuego" 80 50 "Básico" "Fire Spitting"))
(define Torchic (CartaPokemon "Torchic" "Fuego" 60 10 "Básico" "Call the Family"))
...
(EsCartaPokemon? Void)
;; PENDIENTE ;;
;Agregar un nuevo atributo "booleano" que indique si la carta es Ex o no (hay consideración)
